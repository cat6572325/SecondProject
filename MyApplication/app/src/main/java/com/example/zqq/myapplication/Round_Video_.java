...

{
    ...
    public void initView() {
        ...
        timer_flag=(TextView)findViewById(R.id.rounding_time_img);
 ...
 private String randomProdoction() {
    ...
    protected void start() {
        ...
        try {
            ...
            timer_flag.setVisibility(View.VISIBLE);//显示正在录制的标志
            turnC.setVisibility(View.INVISIBLE);
            startTopTimer();
        } catch (Exception e) {
            e.printStackTrace();
...
*/
    ...
    public void applications(View view) {
        Button btn = (Button) view;
        user = new User();

        if (user.applications == null) {
            user.applications = new HashMap<>();

            if (user.applications.containsKey(btn.getText().toString())) {
                btn.setTextColor(0xff000000);
                user.applications.remove(btn.getText().toString());
            } else {
                btn.setTextColor(0xffd0021b);
                user.applications.put(btn.getText().toString(), btn.getText().toString());
            }
        } else {
            ...
            if (user.applications.containsKey(btn.getText().toString())) {
                ...
                if (user.applications.size() > 2) {
                    Toast.makeText(Round_Video_.this, "最多只能选择三个哦", Toast.LENGTH_SHORT).show();
                } else {
                    btn.setTextColor(0xffd0021b);
                    user.applications.put(btn.getText().toString(), btn.getText().toString());
                }
